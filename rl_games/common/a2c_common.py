...
class HybritA2CBase(A2CBase):
    ...
    def __init__(self, base_name, params):
        ...
        num_continuous_actions = sum(1 for action in action_space if not isinstance(action, gym.spaces.Discrete))
        if type(action_space) is gym.spaces.Discrete: #legacy code
        ...
        if type(action_space) is gym.spaces.Tuple:
            ...
            self.actions_num = [action.n if isinstance(action, gym.spaces.Discrete) else action.shape[0] for action in action_space]

            self.bounds_loss_coef = self.config.get('bounds_loss_coef', None)
            self.clip_actions = self.config.get('clip_actions', True)
            # todo introduce device instead of cuda()
            self.actions_low = torch.from_numpy(action_space[-num_continuous_actions].low.copy()).float().to(self.ppo_device)
            self.actions_high = torch.from_numpy(action_space[-num_continuous_actions].high.copy()).float().to(self.ppo_device)
            self.is_multi_discrete = False
            self.is_hybrit = True
