...
from rl_games.algos_torch.transformer import (
    TransformerModel,
    MetamorphModel,
    StatePositionalEncoder,
)
...
class NetworkBuilder:
    ...
    class BaseNetwork(nn.Module):
        def __init__(self, **kwargs):
            ...
            self.activations_factory.register_builder(
                "swish", lambda **kwargs: nn.SiLU(**kwargs)
            )
            self.activations_factory.register_builder(
        ...
        def _build_value_layer(self, input_size, output_size, value_type="legacy"):
            ...
            raise ValueError(
                'value type is not "default", "legacy" or "two_hot_encoded"'
            )
    ...
    class Network(NetworkBuilder.BaseNetwork):
        ...
        def forward(self, obs_dict):
            if self.require_rewards or self.require_last_actions:
                ...
                if self.is_discrete:
                    last_action = torch.nn.functional.one_hot(
                        last_action.long(), num_classes=self.actions_num
                    )
            else:
                obs = obs_dict["obs"]
            if self.permute_input:
...
class TransformerA2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            # Actor is transformer
            self.seq_len = input_shape["transforms"][0]
            ...
            self.value = self._build_value_layer(
                self.transformer["critic_mlp_dim"][-1], self.value_size
            )
            self.value_act = self.activations_factory.create(self.value_activation)
            ...
            if self.is_discrete:
                self.logits = torch.nn.Linear(
                    out_size * input_shape["transforms"][0], actions_num
                )
            if self.is_multi_discrete:
        ...
        def forward(self, obs_dict):
            # Formats the observations
            obs = obs_dict["obs"]
            # We keep the RNN states for compatibility purposes
            states = obs_dict.get("rnn_states", None)
            ...
            if self.is_multi_discrete:
                logits = self.logits(a_out)
                logits = [logits[:, i, :] for i in range(self.seq_len)]
                return logits, value, states
            if self.is_continuous:
...
class MetamorphA2CBuilder(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            self.seq_len = input_shape["transforms"][0]
            net_act = self.activations_factory.create(self.activation)
            self.actor = MetamorphModel(self.transformer, input_shape, net_act)
            out_size = self.transformer["decoder_mlp_dim"][-1]
            # Critic is MLP (we don't need to know the transforms to estimate Q)
            mlp_args = {
                "input_size": input_shape["state"][0],
            ...
            # Value decoder
            self.value = self._build_value_layer(
                self.transformer["state_mlp_dim"][-1], self.value_size
            )
            self.value_act = self.activations_factory.create(self.value_activation)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            if self.use_state_pe:
                self.encoding = StatePositionalEncoder(
                    input_shape["state"][0], self.pe_nfreq, log_space=self.pe_log
                )
...
class MLPDictA2CBuilderHybrit(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            #TODO: Currently only last dimension is continuous and is hardcoded. Create a variable that controls how many contonuous actions there are
            if self.is_hybrit: 
                # Multi discrete
                self.logits = torch.nn.ModuleList(
                    [torch.nn.Linear(out_size, num) for num in actions_num[:-1]]
                )

                #Continuous
                self.mu = torch.nn.Linear(out_size, actions_num[-1])
                self.mu_act = self.activations_factory.create(
                    self.space_config["mu_activation"]
                )
                mu_init = self.init_factory.create(**self.space_config["mu_init"])
                ...
                if self.fixed_sigma:
                    self.sigma = nn.Parameter(
                        torch.zeros(
                            actions_num[-1], requires_grad=True, dtype=torch.float32
                        ),
                        requires_grad=True,
                    )
                else:
                    self.sigma = torch.nn.Linear(out_size, actions_num[-1])
            ...
            if self.is_continuous or self.is_hybrit:
                mu_init(self.mu.weight)
                if self.fixed_sigma:
                    sigma_init(self.sigma)
                else:
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                if self.is_hybrit:
                    ...
                    return logits, mu, sigma, value, states
        ...
        def load(self, params):
            ...
            if self.has_space:
                ...
                self.is_discrete = "discrete" in params["space"]
                self.is_continuous = "continuous" in params["space"]
                self.is_hybrit = "hybrit" in params["space"]
                if self.is_continuous:
                    self.space_config = params["space"]["continuous"]
                    self.fixed_sigma = self.space_config["fixed_sigma"]
                elif self.is_discrete:
                    ...
                    self.space_config = params["space"]["hybrit"]
                    self.fixed_sigma = self.space_config["fixed_sigma"]
            else:
                self.is_discrete = False
                self.is_continuous = False
    ...
    def build(self, name, **kwargs):
        net = MLPDictA2CBuilderHybrit.Network(self.params, **kwargs)
        return net


class MLPDictThrusterA2CBuilder(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
...
class CNNDictA2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            rgb_cnn_input_shape = input_shape["rgb"]
            depth_cnn_input_shape = input_shape["depth"]
            cnn_input_shape = rgb_cnn_input_shape + depth_cnn_input_shape
            cnn_args = {
                "ctype": self.cnn["type"],
            ...
            self.num_vel_repeat = cnn_output_size // (
                self.velocity_dim * self.img_velocity_emb_inv
            )
        ...
        def forward(self, obs_dict):
            ...
            velocity = torch.bmm(
                T_w2b, obs["state"][:, 2:5].unsqueeze(-1).squeeze(-1)
            )  # local velocity (B, 3)
            velocity = velocity.repeat(
                1, self.num_vel_repeat
            )  # (B, 3 * num_vel_repeat)
